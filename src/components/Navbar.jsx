      ...
      <div className="container px-4 mx-auto relative text-sm">
        <div className="flex justify-between items-center">
          <div className="flex items-center flex-shrink-0">
            <Link to={"/"}><img src={logo} alt="logo" className="h-10 mr-2" /></Link>
          </div>
          <ul className="hidden lg:flex ml-14 space-x-12 text-[#49507B]">
            {navItems.map((item, index) => (
              <li key={index}>
                {item.to.startsWith('#') ? (
                  <ScrollLink to={item.to.substring(1)} smooth={true} duration={500} className="hover:cursor-pointer">
                    {item.label}
                  </ScrollLink>
                ) : (
                  <Link to={item.to} className="hover:cursor-pointer">{item.label}</Link>
                )}
              </li>
            ))}
          </ul>
